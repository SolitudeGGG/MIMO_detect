...
# 144 "util.c"
 ...
 int i, j, k, t;
 MyComplex s;
 VITIS_LOOP_162_1: for (i = 0; i < row; i++)
 {
  L[i * col + i].real = 1.0;
  L[i * col + i].imag = 0.0;
 }
 VITIS_LOOP_167_2: for (j = 0; j < col; j++)
 {
  U[j] = A[j];
 }
 VITIS_LOOP_171_3: for (i = 1; i < col; i++)
 {
  L[i * col] = complex_divide(A[i * col], U[0]);

 }
 VITIS_LOOP_176_4: for (k = 1; k < row; k++)
 {
  VITIS_LOOP_178_5: for (j = k; j < col; j++)
  {
   s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_182_6: for (t = 0; t < k; t++) {
    s = complex_add(s, complex_multiply(L[k * col + t], U[t * col + j]));
   }
   U[k * col + j] = complex_subtract(A[k * col + j], s);
  }
  VITIS_LOOP_187_7: for (i = k; i < col; i++)
  {
   s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_191_8: for (t = 0; t < k; t++)
   {
 ...
 VITIS_LOOP_199_9: for (i = 0; i < row; i++)
 {
  L_Inverse[i * col + i].imag = 0.0;
  L_Inverse[i * col + i].real = 1.0;
 }
 VITIS_LOOP_204_10: for (j = 0; j < col; j++)
 {
  VITIS_LOOP_206_11: for (i = j + 1; i < row; i++)
  {
   s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_210_12: for (k = j; k < i; k++) {
 ...
 di.real = 1.0;
 di.imag = 0.0;
 VITIS_LOOP_220_13: for (i = 0; i < col; i++)
 {
  U_Inverse[i * col + i] = complex_divide(di, U[i * col + i]);

 }
 VITIS_LOOP_225_14: for (j = 0; j < col; j++)
 {
  VITIS_LOOP_227_15: for (i = j - 1; i >= 0; i--)
  {
   s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_231_16: for (k = i + 1; k <= j; k++)
   {
...
void my_complex_add(const int n, const MyComplex a[], const MyComplex b[], MyComplex r[]) {
 VITIS_LOOP_246_1: for (int i = 0; i < n; i++) {
...
void my_complex_sub(const int n, const MyComplex a[], const MyComplex b[], MyComplex r[]) {
 VITIS_LOOP_254_1: for (int i = 0; i < n; i++) {
  r[i].real = a[i].real - b[i].real;
  r[i].imag = a[i].imag - b[i].imag;
 }
}
...
void my_complex_copy(const int N, const MyComplex* X, const int incX, MyComplex* Y, const int incY) {
 VITIS_LOOP_270_1: for (int i = 0, j = 0; i < N; i++, j++) {
...
void my_complex_scal(const int N, const float alpha, MyComplex* X, const int incX) {
 VITIS_LOOP_278_1: for (int i = 0; i < N; i++) {
  X[i * incX].real *= alpha;
  X[i * incX].imag *= alpha;
 }
}
...
void c_matmultiple(MyComplex* matA, int transA, MyComplex* matB, int transB, int ma, int na, int mb, int nb, MyComplex* res) {
 ...
 VITIS_LOOP_306_1: for (int i = 0; i < m; i++) {
  VITIS_LOOP_307_2: for (int j = 0; j < n; j++) {
   MyComplex sum = { 0.0f, 0.0f };
   MyComplex temp = { 0.0f, 0.0f };
   VITIS_LOOP_310_3: for (int l = 0; l < k; l++) {
    MyComplex a_element;
                MyComplex b_element;
...
void read_gaussian_data(const char* filename, MyComplex* array, int n, int offset) {
 ...
 VITIS_LOOP_360_1: for (i = 0; i < offset; i++) {
 ...
 VITIS_LOOP_372_2: for (i = 0; i < n; i++) {
  if (fscanf(f, "Real: %f, Imaginary: %f\n", &temp_real, &temp_imag) != 2) {
...
void generateUniformRandoms_int(int Nt, int* x_init, int mu) {
 ...
 VITIS_LOOP_394_1: for (int i = 0; i < Nt; ++i) {
...
void generateUniformRandoms_float(int Nt, float p_uni[9]) {
 VITIS_LOOP_406_1: for (int i = 0; i < Nt; ++i) {
  p_uni[i] = lcg_rand_1();
 }
}
...
int argmin(float* array, int n)
{
 ...
 VITIS_LOOP_416_1: for (i = 1; i < n; i++)
 {
  if (minval > array[i])
  {
   minval = array[i];
...
int unequal_times(int* array1, int* array2, int n)
{
 ...
 VITIS_LOOP_431_1: for (i = 0; i < n; i++)
 {
...
{
 int i;
 VITIS_LOOP_443_1: for (i = 0; i < Nt; i++)
 {
