...
{
 int i;
 VITIS_LOOP_34_1: for (i = 0; i < n * n; i++)
 {

  Mat[i].real = 0.0f;
  Mat[i].imag = 0.0f;
 }
 VITIS_LOOP_40_2: for (i = 0; i < n; i++)
 {
  Mat[i * n + i].real = val;
 }
}
...
{
 int i;
 VITIS_LOOP_50_1: for (i = 0; i < row * col; i++)
 {
...
{
 int i, j;
 VITIS_LOOP_98_1: for (i = 0; i < row; i++)
 {
  VITIS_LOOP_100_2: for (j = 0; j < col; j++)
  {
   A[i * col + j].real = 0.0;
   A[i * col + j].imag = 0.0;
  }
...
void MulMatrix(const MyComplex* A, const MyComplex* B, MyComplex* C, int row, int AB_rc, int col)
{
 int i, j, k;
 MyComplex sum, tmp;
 VITIS_LOOP_114_1: for (i = 0; i < row; i++)
 {
  VITIS_LOOP_116_2: for (j = 0; j < col; j++)
  {

   sum.real = 0.0;
   sum.imag = 0.0;
   VITIS_LOOP_121_3: for (k = 0; k < AB_rc; k++)
   {
...
#pragma HLS PIPELINE II=1
 VITIS_LOOP_163_1: for (i = 0; i < row; i++)
 {
#pragma HLS UNROLL
 L[i * col + i].real = 1.0;
  L[i * col + i].imag = 0.0;
 }
 VITIS_LOOP_169_2: for (j = 0; j < col; j++)
 {
#pragma HLS UNROLL
 U[j] = A[j];
 }
 VITIS_LOOP_174_3: for (i = 1; i < col; i++)
 {
#pragma HLS UNROLL
 L[i * col] = complex_divide(A[i * col], U[0]);

 }
 VITIS_LOOP_180_4: for (k = 1; k < row; k++)
 {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_183_5: for (j = k; j < col; j++)
  {
#pragma HLS PIPELINE II=1
 s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_188_6: for (t = 0; t < k; t++) {
#pragma HLS UNROLL
 s = complex_add(s, complex_multiply(L[k * col + t], U[t * col + j]));
   }
   U[k * col + j] = complex_subtract(A[k * col + j], s);
  }
  VITIS_LOOP_194_7: for (i = k; i < col; i++)
  {
#pragma HLS PIPELINE II=1
 s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_199_8: for (t = 0; t < k; t++)
   {
#pragma HLS UNROLL
 ...
 VITIS_LOOP_208_9: for (i = 0; i < row; i++)
 {
#pragma HLS UNROLL
 L_Inverse[i * col + i].imag = 0.0;
  L_Inverse[i * col + i].real = 1.0;
 }
 VITIS_LOOP_214_10: for (j = 0; j < col; j++)
 {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_217_11: for (i = j + 1; i < row; i++)
  {
#pragma HLS PIPELINE II=1
 s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_222_12: for (k = j; k < i; k++) {
#pragma HLS UNROLL
 ...
 di.real = 1.0;
 di.imag = 0.0;
 VITIS_LOOP_233_13: for (i = 0; i < col; i++)
 {
#pragma HLS UNROLL
 U_Inverse[i * col + i] = complex_divide(di, U[i * col + i]);

 }
 VITIS_LOOP_239_14: for (j = 0; j < col; j++)
 {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_242_15: for (i = j - 1; i >= 0; i--)
  {
#pragma HLS PIPELINE II=1
 s.imag = 0.0;
   s.real = 0.0;
   VITIS_LOOP_247_16: for (k = i + 1; k <= j; k++)
   {
#pragma HLS UNROLL
...
void my_complex_add(const int n, const MyComplex a[], const MyComplex b[], MyComplex r[]) {
 VITIS_LOOP_263_1: for (int i = 0; i < n; i++) {
...
void my_complex_sub(const int n, const MyComplex a[], const MyComplex b[], MyComplex r[]) {
 VITIS_LOOP_271_1: for (int i = 0; i < n; i++) {
  r[i].real = a[i].real - b[i].real;
  r[i].imag = a[i].imag - b[i].imag;
 }
}
...
void my_complex_copy(const int N, const MyComplex* X, const int incX, MyComplex* Y, const int incY) {
 VITIS_LOOP_287_1: for (int i = 0, j = 0; i < N; i++, j++) {
...
void my_complex_scal(const int N, const float alpha, MyComplex* X, const int incX) {
 VITIS_LOOP_295_1: for (int i = 0; i < N; i++) {
  X[i * incX].real *= alpha;
  X[i * incX].imag *= alpha;
 }
}
...
void c_matmultiple(MyComplex* matA, int transA, MyComplex* matB, int transB, int ma, int na, int mb, int nb, MyComplex* res) {
 ...
 VITIS_LOOP_323_1: for (int i = 0; i < m; i++) {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_325_2: for (int j = 0; j < n; j++) {
   MyComplex sum = { 0.0f, 0.0f };
   MyComplex temp = { 0.0f, 0.0f };
   VITIS_LOOP_328_3: for (int l = 0; l < k; l++) {
...
void read_gaussian_data(const char* filename, MyComplex* array, int n, int offset) {
 ...
 VITIS_LOOP_379_1: for (i = 0; i < offset; i++) {
 ...
 VITIS_LOOP_391_2: for (i = 0; i < n; i++) {
  if (fscanf(f, "Real: %f, Imaginary: %f\n", &temp_real, &temp_imag) != 2) {
...
void generateUniformRandoms_int(int Nt, int* x_init, int mu) {
 ...
 VITIS_LOOP_413_1: for (int i = 0; i < Nt; ++i) {
...
void generateUniformRandoms_float(int Nt, float p_uni[9]) {
 VITIS_LOOP_425_1: for (int i = 0; i < Nt; ++i) {
  p_uni[i] = lcg_rand_1();
 }
}
...
int argmin(float* array, int n)
{
 ...
 VITIS_LOOP_435_1: for (i = 1; i < n; i++)
 {
  if (minval > array[i])
  {
   minval = array[i];
...
int unequal_times(int* array1, int* array2, int n)
{
 ...
 VITIS_LOOP_450_1: for (i = 0; i < n; i++)
 {
...
{
 int i;
 VITIS_LOOP_462_1: for (i = 0; i < Nt; i++)
 {
