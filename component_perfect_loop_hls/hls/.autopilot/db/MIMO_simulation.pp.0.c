...
# 2 "<built-in>" 2
# 1 "MIMO_simulation.c" 2
# 1 "./MIMO_simulation.h" 1
...
void Parameter_init(int Nt, int Nr, int mu, int iter, int samples, detect_type_t detect_type, int max_iter)
{
 ...
 bits = (int*)malloc(sizeof(int) * Nt * mu);
 ...
 bits_demod = (int*)malloc(sizeof(int) * Nt * mu);
...
float detection(MIMO_sys_t MIMO_sys, float SNR)
{
 ...
 VITIS_LOOP_221_4: for (i = 0; i < max_iter; i++)
 {

  VITIS_LOOP_224_5: for (j = 0; j < Nr * Nt; j++)
   H[j] = input_H[Nr * Nt * i + j];
  VITIS_LOOP_226_6: for (j = 0; j < Nr; j++)
   y[j] = input_y[Nr * i + j];
  VITIS_LOOP_228_7: for (j = 0; j < Nt * mu; j++)
  ...
  VITIS_LOOP_236_8: for (j = 0; j < Nt * mu; j++)
   fprintf(f, "%d\n", bits_demod[j]);
  fclose(f);
...
float MIMO_detect(MIMO_sys_t MIMO_sys, MyComplex* H, MyComplex* y, MyComplex* x_hat, float SNR)
{
 ...
 case MMSE:
  ...
  detect_time += (end - start) / (float)((__clock_t) 1000000);
  break;

 case MHGD:
  start = clock();

  MSE = MHGD_detect_accel(x_hat, Nt, Nr, mu, H, y, sigma2, mmse_init, lr_approx, samples, v_tb);
...
void _16QAM_Demodulation(MyComplex* x_hat, int Nt, int* bits_demod)
{
 ...
 VITIS_LOOP_381_1: for (i = 0; i < Nt; i++)
 {
  VITIS_LOOP_383_2: for (j = 0; j < 16; j++)
  {
  ...
  VITIS_LOOP_389_3: for (j = 0; j < 16; j++)
  {
   distance[j] = sqrt(temp[j].real * temp[j].real + temp[j].imag * temp[j].imag);
  }
...
void _64QAM_Demodulation(MyComplex* x_hat, int Nt, int* bits_demod)
{
 ...
 VITIS_LOOP_411_1: for (i = 0; i < Nt; i++)
 {
  VITIS_LOOP_413_2: for (j = 0; j < 64; j++)
  {
