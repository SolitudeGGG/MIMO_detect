...
__attribute__((sdx_kernel("MHGD_detect_accel", 0))) float MHGD_detect_accel(MyComplex* x_hat, int Nt, int Nr, int mu, MyComplex* H, MyComplex* y, float sigma2, int mmse_init, int lr_approx, int iter, MyComplex* v_tb);
...
void MHGD_free_accel(int Nt, int Nr, int mu)
{
 ...
 VITIS_LOOP_190_1: for (i = 0; i < mu; i++)
  free(table_real[i]);
 VITIS_LOOP_192_2: for (i = 0; i < mu; i++)
  free(table_imag[i]);
 VITIS_LOOP_194_3: for (i = 0; i < Nt; i++)
  free(temp_real[i]);
 VITIS_LOOP_196_4: for (i = 0; i < Nt; i++)
  free(temp_imag[i]);
...
__attribute__((sdx_kernel("MHGD_detect_accel", 0))) float MHGD_detect_accel(MyComplex* x_hat, int Nt, int Nr, int mu, MyComplex* H, MyComplex* y, float sigma2, int mmse_init, int lr_approx, int iter, MyComplex* v_tb)
{
#line 74 "/home/ggg_wufuqi/hls/MIMO_detect-main/run_tcl.tcl"
#pragma HLSDIRECTIVE TOP name=MHGD_detect_accel
# 215 "MHGD_accel.c"
...
# 406 "MHGD_accel.c"
 ...
 {
  VITIS_LOOP_427_1: for (i = 0; i < Nr * Nr; i++)
  {
 ...
 else
 {
  ...
  VITIS_LOOP_449_2: for (i = 0; i < Nt; i++)
  {
#pragma HLS LOOP_TRIPCOUNT max=Nt_max min=Nt_min
 ...
 step_size = (((dqam) > (sqrt(r_norm / (float)Nr))) ? (dqam) : (sqrt(r_norm / (float)Nr))) * alpha;

 int offset = 0;



 VITIS_LOOP_483_3: for (k = 0; k < iter; k++)
 {
...
#pragma HLS PIPELINE II=1
 ...
 c_matmultiple(H, transA, r, transB, Nr, Nt, Nr, 1, temp_Nt);
  ...
  VITIS_LOOP_494_4: for(int i = 0; i < Nt; i++){
#pragma HLS LOOP_TRIPCOUNT max=Nt_max min=Nt_min
 v[i].real = v_tb[i+offset].real;
  ...
  log_pacc = (((0) < (-(r_norm_prop - r_norm))) ? (0) : (-(r_norm_prop - r_norm)));
  p_acc = exp(log_pacc);
...
void Hx_accel(int Nt, int Nr, int mu, float dqam, MyComplex* x, MyComplex* x_result)
{
 ...
 VITIS_LOOP_559_1: for (i = 0; i < Nt; i++)
 {
  Hx_table_id = (x[i].real / dqam + mu - 1) / 2;



  VITIS_LOOP_565_2: for (j = 0; j < Nr; j++)
  {
  ...
  VITIS_LOOP_573_3: for (j = 0; j < Nr; j++)
  {
   temp_imag[i][j] = table_imag[Hx_table_id][Nt * j + i];
  }
 }
# 591 "MHGD_accel.c"
 VITIS_LOOP_591_4: for (i = 0; i < Nt; i++)
 {
